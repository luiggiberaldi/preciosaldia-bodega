    ...
    const paymentBreakdown = allTodayTransactions.reduce((acc, s) => {
        if (s.payments && s.payments.length > 0) {
            s.payments.forEach(p => {
                acc[p.methodId] = (acc[p.methodId] || 0) + p.amountBs;
            });
        } else {
            const method = s.paymentMethod || 'efectivo_bs';
            acc[method] = (acc[method] || 0) + (s.totalBs || 0);
        }
        return acc;
    }, {});
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 shadow-sm mb-5">
                    ...
                    <div className="space-y-2">
                        {Object.entries(paymentBreakdown).map(([method, total]) => {
                            ...
                            const pct = todayTotalBs > 0 ? (total / todayTotalBs * 100) : 0;
                            return (
                                <div key={method}>
                                    <div className="flex justify-between text-sm mb-1">
                                        <span className="text-slate-600 dark:text-slate-300 font-medium">{label}</span>
                                        <span className="font-bold text-slate-700 dark:text-white">{formatBs(total)} Bs</span>
                                    </div>
