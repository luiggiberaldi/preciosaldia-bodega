            ...
            <div className="flex-1 overflow-y-auto overscroll-contain pb-28">
                    ...
                    {discountData?.active && (
                        <div className="flex flex-col items-center justify-center space-y-1 mb-3 pb-3 border-b border-slate-200/50 dark:border-slate-800/50">
                            <div className="flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400">
                                <span>Subtotal:</span>
                                <span>{copEnabled && tasaCop > 0 ? `$${cartSubtotalUsd.toFixed(2)} · ${formatCop(cartSubtotalUsd * tasaCop)} COP · ${formatBs(cartSubtotalBs)} Bs` : `$${cartSubtotalUsd.toFixed(2)}`}</span>
                            </div>
                            <div className="flex items-center gap-2 text-sm font-black text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-900/20 px-3 py-1 rounded-lg">
                                ...
                                <span>{copEnabled && tasaCop > 0 ? `-$${discountData.amountUsd.toFixed(2)} / -${formatCop(discountData.amountUsd * tasaCop)} COP` : `-$${discountData.amountUsd.toFixed(2)}`}</span>
                            </div>
                        </div>
                    )}
                ...
                {selectedCustomer?.deuda < -0.01 && remainingUsd > 0.01 && (
                    <div className="px-3 py-1">
                        ...
                        >
                            <Wallet size={16} /> Usar Saldo a Favor ({copEnabled && tasaCop > 0 ? `$${Math.abs(selectedCustomer.deuda).toFixed(2)} / ${formatCop(Math.abs(selectedCustomer.deuda) * tasaCop)} COP` : `$${Math.abs(selectedCustomer.deuda).toFixed(2)}`})
                        </button>
                    </div>
                )}
            </div>
            ...
            <div className="shrink-0 px-4 py-3 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-950 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
                ...
                >
                    {isPaid ? (
                        ...
                        <><Users size={18} /> FIAR RESTANTE ({copEnabled && tasaCop > 0 ? `$${remainingUsd.toFixed(2)} / ${formatCop(remainingUsd * tasaCop)} COP` : `$${remainingUsd.toFixed(2)}`})</>
                    ) : (
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-sm sm:max-w-md w-full shadow-2xl border border-slate-200 dark:border-slate-800" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4 sm:p-5 mb-5">
                            <div className="text-center mb-3">
                                ...
                                <p className="text-sm sm:text-base font-bold text-amber-500/70 mt-0.5">{copEnabled && tasaCop > 0 ? `${formatCop(remainingUsd * tasaCop)} COP · ` : ''}{formatBs(remainingBs)} Bs</p>
                            </div>
                            <div className="border-t border-amber-200/50 dark:border-amber-800/20 pt-3 space-y-2">
                                ...
                                {totalPaidUsd > 0.01 && (
                                    <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">
                                        El cliente abona <span className="font-bold text-emerald-600">{copEnabled && tasaCop > 0 ? `$${totalPaidUsd.toFixed(2)} / ${formatCop(totalPaidUsd * tasaCop)} COP` : `$${totalPaidUsd.toFixed(2)}`}</span> ahora y el restante queda pendiente.
                                    </p>
                                )}
                                {totalPaidUsd <= 0.01 && (
                                    <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400">
                                ...
                                {selectedCustomer && (selectedCustomer.deuda || 0) > 0.01 && (
                                    <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/30 rounded-lg p-2.5 mt-2">
                                        <p className="text-[11px] sm:text-xs font-bold text-red-600 dark:text-red-400">
                                            Este cliente ya tiene una deuda de {copEnabled && tasaCop > 0 ? `$${(selectedCustomer.deuda || 0).toFixed(2)} (${formatCop((selectedCustomer.deuda || 0) * tasaCop)} COP)` : `$${(selectedCustomer.deuda || 0).toFixed(2)}`}. La deuda total pasara a ser {copEnabled && tasaCop > 0 ? `$${((selectedCustomer.deuda || 0) + remainingUsd).toFixed(2)} (${formatCop(((selectedCustomer.deuda || 0) + remainingUsd) * tasaCop)} COP)` : `$${((selectedCustomer.deuda || 0) + remainingUsd).toFixed(2)}`}.
                                        </p>
                                    </div>
                                )}
                            </div>
